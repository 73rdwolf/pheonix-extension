document.addEventListener('DOMContentLoaded', async () => {
  ...
  // --- GLOBAL STATE FIX ---
  // These variables are critical for the calendar module and must be available globally within this scope.
  // Their absence was causing the calendar to fail rendering (ReferenceError).
  let localEvents = [];
  ...
  await new Promise((resolve) => {
    chrome.storage.local.get(['theme_preference', 'theme_mode', 'gradient_enabled', 'gradient_color_1', 'gradient_color_2', 'clock_swap_enabled', 'font_hud_preference', 'solid_bg_color'], (result) => {
  ...
  // Check auth non-interactively on load
  try { checkAuthStatus(); } catch (e) { console.error("Auth check failed", e); }
  ...
  function initOnboardingLogic() {
    ...
    if (googleConnectBtn) {
      googleConnectBtn.addEventListener('click', async () => {
        console.log('[Onboarding] Google Connect clicked');
        try {
          ...
          chrome.identity.getAuthToken({ interactive: true }, async (token) => {
            if (chrome.runtime.lastError || !token) {
              console.error('[Onboarding] Google auth error:', chrome.runtime.lastError?.message);
            ...
            if (connectSuccess) {
              ...
              // Update success message based on connection type
            ...
            console.log('[Onboarding] Google connected successfully');
          });
        } catch (error) {
...
async function refreshAuthToken() {
  console.log("[Google API] Attempting silent token refresh...");
  // Trust Chrome's Identity API - it handles refresh tokens automatically
  const token = await attemptSilentRefresh();
  if (token) {
    console.log("[Google API] Token refreshed automatically by Chrome.");
    // Update stored token
    await new Promise(resolve => chrome.storage.local.set({ "google_access_token": token }, resolve));
    return token;
  }
  ...
  if (storage.isLoggedIn && storage.google_user_persistent) {
    ...
    // Clear logged-in state - user will need to re-authenticate manually
    await new Promise(resolve => chrome.storage.local.set({ "isLoggedIn": false }, resolve));
  }
...
async function updateProfileInSettings(token, retryCount = 0) {
  ...
  try {
    ...
    // Use googleApiFetch which handles token refresh automatically
...
function initSettings() {
  ...
  function applyColor(color) {
    ...
    localStorage.setItem('solid_bg_color', finalColor);
...
const SyncQueue = {
  ...
  async processQueue(token) {
    ...
    for (const item of this.queue) {
      try {
        if (item.type === 'task') {
          // ... (Task logic preserved if not modifying, but I'll rewrite to be safe)
          ...
          if (item.action === 'create') {
            const { id: _, ...safePayload } = item.data.resource || {}; // Strip temp ID
            ...
            // Update local cache to replace temp ID with real ID
  ...
  if (window.PerformanceUtils) {
    window.PerformanceUtils.batchDOMUpdate(() => {
      ...
      if (activeTasks.length === 0) {
        ...
        const activeFragment = window.PerformanceUtils.createFragment();
      ...
      // Batch create completed tasks
      if (completedTasks.length === 0) {
        completedListEl.innerHTML = '<div class="empty-state">No completed tasks.</div>';
      } else {
        const completedFragment = window.PerformanceUtils.createFragment();
    ...
    taskListEl.innerHTML = activeTasks.length === 0 ? '<div class="empty-state">No active objectives.</div>' : '';
    activeTasks.forEach(task => {
      taskListEl.appendChild(createTaskCard(task));
    });

    completedListEl.innerHTML = completedTasks.length === 0 ? '<div class="empty-state">No completed tasks.</div>' : '';
    completedTasks.forEach(task => {
      completedListEl.appendChild(createTaskCard(task, true));
    });
...
function createTaskCard(task, isCompleted = false) {
  ...
  // Create actions container with accessibility
  const actions = document.createElement('div');
  actions.className = 'task-actions';
  actions.setAttribute('role', 'toolbar');
  actions.setAttribute('aria-label', 'Task actions');

  // Create complete button (first action)
  const completeBtn = document.createElement('button');
  completeBtn.className = 'action-btn complete-task-btn';
  completeBtn.title = isCompleted ? 'Mark Incomplete' : 'Complete';
  completeBtn.setAttribute('aria-label', isCompleted ? 'Mark task incomplete' : 'Mark task complete');
  completeBtn.setAttribute('tabindex', '0');
  completeBtn.type = 'button';
  completeBtn.innerHTML = isCompleted
    ? '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12l6 6L21 6"/></svg>'
  ...
  copyBtn.setAttribute('aria-label', 'Copy task note');
  ...
  const actionButtons = [completeBtn, copyBtn, deleteBtn];
  actionButtons.forEach((btn, i) => {
    btn.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
        e.preventDefault();
        const next = actionButtons[(i + 1) % actionButtons.length];
        next.focus();
      } else if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
        e.preventDefault();
        const prev = actionButtons[(i - 1 + actionButtons.length) % actionButtons.length];
        prev.focus();
      } else if (e.key === 'Escape') {
        card.classList.remove('actions-visible', 'touch-active');
        card.focus();
      }
    });
  });
...
/* --- LINK PASTE TO TASK CARD --- */
/**
 * Detects URL paste events and auto-creates link task cards
 * Features: title fetch, quick date selector, debounce, offline fallback
 */
const LinkPasteHandler = {
  ...
  async createLinkTask(url) {
    ...
    // Store for date selector
    this.currentLinkTaskId = taskData.id;

    // Add to task list (at top)
    activeTasks.unshift(taskData);
    saveLocalTasks();

    // Render with animation
    this.renderLinkCard(taskData);

    // Show notification
    showNotification('LINK SAVED', 'success');

    // Show quick date selector overlay
    this.showDateOverlay();

    // Sync with Google if connected
    this.syncLinkToGoogle(taskData);
  },

  // Render link card into task list
  renderLinkCard(task) {
    const taskListEl = document.getElementById('task-list');
    if (!taskListEl) return;

    // Remove empty state if present
    const emptyState = taskListEl.querySelector('.empty-state');
    if (emptyState) emptyState.remove();

    // Create card element
    const card = document.createElement('div');
    card.className = 'task-card link-card animate-in';
    card.dataset.id = task.id;
    card.draggable = true;

    // Play/Open icon SVG
    const playIcon = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    card.innerHTML = `
      <div class="task-main">
        ...
        <div class="link-card-title">
          <span class="link-icon">${playIcon}</span>
          <span class="link-title-text" title="${task.title}">${task.title}</span>
        </div>
        <div class="task-actions">
          <button class="action-btn copy-task-btn" title="Copy URL">
          ...
          <button class="action-btn delete-task-btn" title="Delete">×</button>
        </div>
      </div>
      <a href="${task.linkUrl}" target="_blank" rel="noopener noreferrer" class="link-url" title="${task.linkUrl}">${task.linkUrl}</a>
    `;

    // Link click handler (prevent card expansion)
    ...
    // Insert at top
    taskListEl.insertBefore(card, taskListEl.firstChild);

    // Remove animation class after animation completes
    setTimeout(() => card.classList.remove('animate-in'), 300);
  },

  // Initialize quick date overlay listeners
  initDateOverlay() {
    const overlay = document.getElementById('quick-date-overlay');
    if (!overlay) return;

    overlay.querySelectorAll('.quick-date-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        this.handleDateSelect(btn.dataset.offset);
      });
    });
  ...
  // Show date overlay for 3 seconds
  showDateOverlay() {
    const overlay = document.getElementById('quick-date-overlay');
    if (!overlay) return;

    // Clear any existing timeout
    if (this.dateOverlayTimeout) {
      clearTimeout(this.dateOverlayTimeout);
    }
  ...
  hideDateOverlay() {
    ...
    if (this.dateOverlayTimeout) {
      clearTimeout(this.dateOverlayTimeout);
      this.dateOverlayTimeout = null;
    }
  },

  // Handle date selection
  handleDateSelect(offset) {
    if (!this.currentLinkTaskId) return;

    const task = activeTasks.find(t => t.id === this.currentLinkTaskId);
    if (!task) return;

    let dueDate;

    if (offset === 'custom') {
      // For custom, use browser date picker prompt
      ...
      const offsetDays = parseFloat(offset);
      dueDate = new Date();
      if (offsetDays < 1) {
        // Add hours (e.g., 0.25 = 6 hours)
        dueDate.setHours(dueDate.getHours() + Math.round(offsetDays * 24));
      } else {
        // Add days
    ...
    if (card) {
      ...
      if (!dueEl) {
        ...
        dueEl.className = 'task-due';
        card.querySelector('.task-main').appendChild(dueEl);
      }
      dueEl.textContent = dateStr;
    }

    showNotification(`DUE: ${dueDate.toLocaleDateString()}`, 'info');
    this.hideDateOverlay();

    // Sync due date to Google if connected
    this.updateDueDateOnGoogle(task.id, dueDate);
  },

  // Sync new link task to Google Tasks (uses ensureToken for persistent auth)
  async syncLinkToGoogle(task) {
    try {
      await googleApiRequest('https://www.googleapis.com/tasks/v1/lists/@default/tasks', {
        method: 'POST',
      ...
      console.error('Failed to sync link task to Google:', err);
    }
  },

  // Update due date on Google Tasks (uses ensureToken for persistent auth)
  async updateDueDateOnGoogle(taskId, dueDate) {
    // Only sync real Google task IDs (not local link- prefixed ones)
    if (taskId.startsWith('link-')) return;

    try {
      await googleApiRequest(`https://www.googleapis.com/tasks/v1/lists/@default/tasks/${taskId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ due: dueDate.toISOString() })
      });
    } catch (err) {
      console.error('Failed to update due date on Google:', err);
    }
  }
};
...
async function checkAuthStatus() {
  ...
  try {
    ...
    // If user has a stored token OR is marked as logged in, try to verify/refresh
    // This handles cases where token exists but isLoggedIn flag might be missing
    if (storedToken || isLoggedIn || isPersistent) {
      console.log("[Auth] User appears to be logged in. Verifying session with silent auth...");

      // First, try to get a fresh token silently using Chrome's Identity API
      // Chrome automatically handles refresh tokens in the background
      const token = await attemptSilentRefresh();

      if (token) {
        console.log("[Auth] Session still valid. Token refreshed automatically by Chrome.");
        // Save token and ensure logged-in state is set
        await chrome.storage.local.set({
          "google_access_token": token,
          "isLoggedIn": true,
          "google_user_persistent": true
        ...
        // Silent refresh failed, but we have a stored token
        // Try to validate the stored token - it might still be valid
        console.log("[Auth] Silent refresh failed, but stored token exists. Validating stored token...");
        try {
          const controller = new AbortController();
          const timeoutId = setTimeout(() => controller.abort(), 5000);

          const validateRes = await fetch(`https://www.googleapis.com/oauth2/v3/tokeninfo?access_token=${storedToken}`, {
          ...
          if (validateRes.ok) {
            ...
            if (data.error !== 'invalid_token' && data.error !== 'invalid_request') {
              // Stored token is still valid
              console.log("[Auth] Stored token is still valid.");
              await chrome.storage.local.set({
                "google_access_token": storedToken,
                "isLoggedIn": true,
                "google_user_persistent": true
              });
              hideConnectUI();
              await updateProfileInSettings(storedToken);
              syncWorkspace(storedToken);
              return;
            }
          }
        } catch (err) {
          console.warn("[Auth] Token validation error (network issue?):", err.message);
          // On network errors, assume token might be valid and keep it
          if (storedToken) {
            await chrome.storage.local.set({
              "isLoggedIn": true,
              "google_user_persistent": true
...
// Silent refresh using chrome.identity.getAuthToken
const SCOPES = [
  "https://www.googleapis.com/auth/gmail.readonly",
  "https://www.googleapis.com/auth/gmail.modify",
  "https://www.googleapis.com/auth/gmail.send",
...
// Silent refresh - for persistent users, falls back to interactive if silent fails
...
function initModuleResizing() {
  ...
  if (savedHeight) {
    taskModule.style.setProperty('height', savedHeight + 'px', 'important');
    taskModule.style.setProperty('min-height', savedHeight + 'px', 'important');
  }
