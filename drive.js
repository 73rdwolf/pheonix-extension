...
function renderDriveFiles(files, token = null) {
    ...
    const htmlContent = files.map(item => {
        ...
        // Broaden check: explicitly handle shortcuts as folders if they point to one, but since we don't have target info, 
        // we'll stick to strict folder but fix the Visual Weight difference.
        const isFolder = item.mimeType === 'application/vnd.google-apps.folder';
        const isPublic = item.isPublic === true;

        // Wrap icon in a fixed-width container for perfect alignment
        const iconWrapper = `<span style="display:inline-flex; align-items:center; justify-content:center; width:20px; margin-right:8px;">${isFolder
            ...
            : '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="opacity:0.75;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>'}</span>`; /* File Text Icon */
        ...
        // Using existing class structure for styling compatibility
        return `
    ...
    console.log('[Drive] HTML content set. List element now has', listEl.children.length, 'children');

    // Verify the structure
    if (listEl.children.length === 0 && files.length > 0) {
        console.error('[Drive] WARNING: HTML was set but no children found! HTML length:', htmlContent.length);
    }
