...
async function validateAndRefreshTokenOnStartup() {
    console.log('[Background] ========== STARTUP AUTH CHECK ==========');
    console.log('[Background] Verifying Google auth on startup...');
    try {
        // Check if user is logged in OR has a stored token
        const storage = await chrome.storage.local.get(["isLoggedIn", "google_access_token", "google_user_persistent", "google_token_expiry"]);
        const isLoggedIn = storage.isLoggedIn;
        const storedToken = storage.google_access_token;
        const isPersistent = storage.google_user_persistent;
        const tokenExpiry = storage.google_token_expiry;

        console.log('[Background] Startup auth state:', {
        ...
        if (!storedToken && !isLoggedIn && !isPersistent) {
            console.log('[Background] User not logged in (no token, not logged in, not persistent).');
            return;
        }
        ...
        console.log('[Background] Step 1: Attempting silent auth (interactive: false)...');
        chrome.identity.getAuthToken({ interactive: false, scopes: SCOPES }, (token) => {
            if (chrome.runtime.lastError || !token) {
                console.log('[Background] Step 1 FAILED - Silent auth failed:', chrome.runtime.lastError?.message);

                // For persistent users, try interactive auth before falling back to UI
                if (isPersistent || isLoggedIn) {
                    console.log('[Background] Step 2: User is persistent/logged-in. Trying interactive auth...');
                    console.log('[Background] (Interactive auth often succeeds silently if user is logged into Chrome)');
                    ...
                    chrome.identity.getAuthToken({ interactive: true, scopes: SCOPES }, (interactiveToken) => {
                        if (chrome.runtime.lastError || !interactiveToken) {
                            console.log('[Background] Step 2 FAILED - Interactive auth failed:', chrome.runtime.lastError?.message);
                            // Step 3: Keep flags for UI to handle manual reconnection as last resort
                            console.log('[Background] Step 3: Both silent and interactive auth failed!');
                            console.log('[Background] Keeping persistent flags for manual reconnection...');
                            chrome.storage.local.set({
                                "isLoggedIn": true,
                                "google_user_persistent": true
                            });
                            // Notify UI to trigger tab-based auth (user interaction required)
                            console.log('[Background] Sending AUTO_RECONNECT_NEEDED message to UI...');
                            chrome.runtime.sendMessage({
                                type: 'AUTO_RECONNECT_NEEDED'
                            }).catch((err) => {
                                console.log('[Background] AUTO_RECONNECT_NEEDED message not received (UI may not be open):', err?.message);
                            });
                        } else {
                            // Success! Token refreshed via Chrome's Identity API
                            console.log('[Background] Step 2 SUCCESS - Interactive auth succeeded!');
                            console.log('[Background] Token refreshed automatically via Chrome Identity API.');
                            const authTimestamp = Date.now();
                            const tokenExpiryTime = authTimestamp + (3500 * 1000);
                            chrome.storage.local.set({
                            ...
                            console.log('[Background] Token saved. New expiry:', new Date(tokenExpiryTime).toISOString());
                ...
                console.log('[Background] Step 1 SUCCESS - Silent auth succeeded!');
                console.log('[Background] Token refreshed automatically by Chrome.');
            ...
            console.log('[Background] ========== STARTUP AUTH COMPLETE ==========');
        });
...
async function refreshToken(oldToken) {
    console.log('[Background] === refreshToken called ===');
    return new Promise((resolve) => {
        try {
            // First, remove the invalid token from cache if provided
            if (oldToken) {
                console.log('[Background] Removing stale token from cache (length:', oldToken.length, ')');
                chrome.identity.removeCachedAuthToken({ token: oldToken }, () => {
                    // Continue even if removal fails
                    if (chrome.runtime.lastError) {
                        console.warn('[Background] Failed to remove cached token (continuing anyway):', chrome.runtime.lastError?.message);
                    } else {
                        console.log('[Background] Stale token removed from cache successfully');
                    }
                    // Now fetch a new one
                    console.log('[Background] Fetching new token...');
                    fetchNewToken(resolve);
                });
            } else {
                console.log('[Background] No old token to remove, fetching new token directly...');
                fetchNewToken(resolve);
            }
        } catch (e) {
...
function fetchNewToken(resolve) {
    ...
    console.log('[Background] fetchNewToken: Trying silent getAuthToken...');
    chrome.identity.getAuthToken({ interactive: false, scopes: SCOPES }, async (token) => {
        if (chrome.runtime.lastError || !token) {
            console.log('[Background] fetchNewToken: Silent getAuthToken FAILED:', chrome.runtime.lastError?.message);
            ...
            const isPersistent = authVars.google_user_persistent;
            console.log('[Background] fetchNewToken: isPersistent =', isPersistent);

            if (isPersistent) {
                console.log('[Background] fetchNewToken: Persistent user - trying interactive auth...');
                // Try interactive auth for persistent users
                chrome.identity.getAuthToken({ interactive: true, scopes: SCOPES }, (interactiveToken) => {
                    if (chrome.runtime.lastError || !interactiveToken) {
                        console.log('[Background] fetchNewToken: Interactive auth FAILED:', chrome.runtime.lastError?.message);
                        console.log('[Background] fetchNewToken: Falling back to tab-based auth...');
                        // Fall back to tab-based auth flow
                        tryTabBasedAuth(SCOPES, resolve);
                    } else {
                        console.log('[Background] fetchNewToken: Interactive auth SUCCEEDED!');
            ...
            console.log('[Background] fetchNewToken: Non-persistent user - trying silent WebAuthFlow...');
            chrome.identity.launchWebAuthFlow({
                url: authUrl.toString(),
                interactive: false
            }, (responseUrl) => {
                if (chrome.runtime.lastError || !responseUrl) {
                    console.log('[Background] fetchNewToken: Silent WebAuthFlow FAILED:', chrome.runtime.lastError?.message);
                    console.log('[Background] fetchNewToken: ALL AUTH METHODS FAILED - returning null');
                    resolve(null);
                    return;
                }
                console.log('[Background] fetchNewToken: Silent WebAuthFlow returned URL');
                const url = new URL(responseUrl);
                const params = new URLSearchParams(url.hash.substring(1));
                const accessToken = params.get('access_token');
                if (accessToken) {
                    console.log('[Background] fetchNewToken: Extracted token from WebAuthFlow');
                    saveAndResolveToken(accessToken, resolve);
                } else {
                    console.log('[Background] fetchNewToken: No token in WebAuthFlow response');
                    resolve(null);
                }
            });
            return;
        }
        console.log('[Background] fetchNewToken: Silent getAuthToken SUCCEEDED!');
...
function saveAndResolveToken(token, resolve) {
    console.log('[Background] saveAndResolveToken: Saving new token...');
    ...
    chrome.storage.local.set({
        ...
        console.log('[Background] saveAndResolveToken: Token SAVED successfully!');
        console.log('[Background] Token length:', token.length);
        console.log('[Background] New expiry:', new Date(tokenExpiryTime).toISOString());
        console.log('[Background] isLoggedIn set to: true');
        resolve(token);
    });
}
...
async function ensureToken() {
    ...
    console.log('[Auth] ensureToken state:', {
        hasToken: !!storage.google_access_token,
        tokenExpiry: storage.google_token_expiry ? new Date(storage.google_token_expiry).toISOString() : 'none',
        isExpired: isExpired(storage.google_token_expiry),
        isPersistent: storage.google_user_persistent
    });

    if (!storage.google_access_token) {
        console.warn('[Auth] ensureToken: No token found in storage!');
        throw new Error("NO_TOKEN");
    }

    // If token expires within 5 minutes, refresh proactively
    if (isExpired(storage.google_token_expiry)) {
        console.log('[Auth] ensureToken: Token expiring soon, refreshing proactively...');
        const newToken = await refreshToken(storage.google_access_token);
        if (newToken) {
            console.log('[Auth] ensureToken: Token refreshed successfully!');
            return newToken;
        }
        // If refresh fails, return old token (might still work briefly)
        console.warn('[Auth] ensureToken: Refresh FAILED, using existing token (may fail)...');
        return storage.google_access_token;
    }

    console.log('[Auth] ensureToken: Token is valid, returning existing token');
    return storage.google_access_token;
}
